...
*/        var _suppressOverwrites, _globalTimeline, _win, _coreInitted, _doc, _coreReady, _lastRenderedFrame, _quickTween, _tickerActive, _config = {
        ...
        _easeMap.Back, _easeMap.SteppedEase, _easeMap.Bounce, _easeMap.Sine, _easeMap.Expo, 
        _easeMap.Circ;
        /*!
 * CSSPlugin 3.10.4
 * https://greensock.com
...
*/
        ...
        gsapWithCSS.registerPlugin({
            ...
            register(gsap, Plugin) {
                var CSSPlugin = gsap.plugins.css, cssCore = CSSPlugin.core || console.warn("Requires GSAP 3.2.1 or later") || {}, _removeLinkedListItem = gsap.core._removeLinkedListItem, _removeProperty = cssCore._removeProperty, PropTween = gsap.core.PropTween, _getAllStyles = function(target, uncache) {
                    var p, styles = {}, computed = getComputedStyle(target), cache = target._gsap;
                    for (p in computed) if (isNaN(p) && "cssText" !== p && "length" !== p) styles[p] = computed[p];
                    uncache && cache && (cache.uncache = true);
                    gsap.getProperty(target, "x");
                    cache = target._gsap;
                    for (p in cache) styles[p] = cache[p];
                    return styles;
                };
                Plugin.prototype.init = function(target, endValue, tween) {
                    let endVars, p, startClassList = target.getAttribute("class"), style = target.style, cssText = style.cssText, cache = target._gsap, classPT = cache.classPT, inlineToRemoveAtEnd = {}, end = "=" !== endValue.charAt(1) ? endValue : startClassList.replace(new RegExp("(?:\\s|^)" + endValue.substr(2) + "(?![\\w-])"), "") + ("+" === endValue.charAt(0) ? " " + endValue.substr(2) : ""), plugin = this, changingVars = {}, startVars = _getAllStyles(target), transformRelated = /(transform|perspective)/i, css = new CSSPlugin, _renderClassName = function(ratio) {
                        css.render(ratio, css);
                        if (!ratio || 1 === ratio) {
                            target.setAttribute("class", ratio ? end : startClassList);
                            for (var p in inlineToRemoveAtEnd) _removeProperty(target, p);
                        }
                    };
                    if (classPT) {
                    ...
                    for (p in endVars) if (endVars[p] !== startVars[p] && !transformRelated.test(p)) {
                        changingVars[p] = endVars[p];
                        if (!style[p] && "0" !== style[p]) inlineToRemoveAtEnd[p] = 1;
                    }
                    cache.classPT = plugin._pt = new PropTween(null, target, "className", 0, 0, _renderClassName, plugin, 0, -11);
                    if (style.cssText !== cssText) style.cssText = cssText;
                    cache.uncache = true;
                    gsap.getProperty(target, "x");
        ...
        function pageAnimOut(container) {
            return tl.from(".preloader_top", {
                duration: .6,
                scaleX: 1,
                transformOrigin: "top right"
            }).from(".preloader_bottom", {
                duration: .6,
                scaleX: 1,
        ...
        function animContent() {
            return tl.to(".doves", {
                ...
                opacity: 1,
                stagger: .2
            }, "-=0.3").fromTo(".screen__navigation", {
                translateY: 150,
                opacity: 0
            }, {
                translateY: 0,
                opacity: 1
            }, "-=0.4").fromTo(".screen__feathers", {
                scale: 0,
                opacity: 0,
                duration: 1
            }, {
        ...
        barba_umd.init({
            ...
            views: [ {
                ...
                beforeEnter() {
                    ...
                    if (mainPage) {
                        const video = mainPage.querySelector(".start-screen__bg video");
                        video.play();
                    }
                }
            } ]
